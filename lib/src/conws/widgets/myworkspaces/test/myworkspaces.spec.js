...
define(['csui/lib/jquery', 'csui/lib/underscore',
  ...
  var selectorViewItems                    = '.binf-list-group-item',
      ...
      selectorFavorites                    = '.icon-socialFav',
      selectorFavoritesOpen                = '.icon-socialFavOpen',
      ...
      selectorExpandedViewItems            = '[role="row"].csui-saved-item',
      ...
      expandedColumn                       = 'csui-table-cell-',
      selectorExpandedColumn               = '.' + expandedColumn,
  ...
  describe('MyWorkspacesView', function () {
    ...
    describe("in expanded state", function () {
      ...
      describe("show", function () {
        ...
        it("name, type, size, modify date an favorite column", function (done) {
          ...
          TestUtil.run(done,function () {
            ...
            var rowHead = $(tableHead).find("tr");
            var colName = $(tableHead).find(selectorExpandedColumn + 'name');
            var colType = $(tableHead).find(selectorExpandedColumn + 'type');
            var colSize = $(tableHead).find(selectorExpandedColumn + 'size');
            var colDate = $(tableHead).find(selectorExpandedColumn + 'date');
            var colFavo = $(tableHead).find(selectorExpandedColumn + 'favorite');
            var colCols = $(tableHead).find('[class^=' + expandedColumn + ']');
            expect($(tableHead).find(selectorExpandedColumn + 'name').length).toEqual(1);
            expect($(tableHead).find(selectorExpandedColumn + 'type').length).toEqual(1);
            expect($(tableHead).find(selectorExpandedColumn + 'size').length).toEqual(1);
            expect($(tableHead).find(selectorExpandedColumn + 'date').length).toEqual(1);
            expect($(tableHead).find(selectorExpandedColumn + 'favorite').length).toEqual(1);
            expect($(tableHead).find('[class^=' + expandedColumn + ']').length).toEqual(5);
            tableView.$el.remove();
          });
        });
          ...
          TestUtil.run(done,function () {
            function hasColumn(tableElement,fieldid) {
              ...
              if (headRows.length>0) {
                if ($(headRows[0]).find('.csui-table-cell-'+fieldid).length>0) {
            ...
            function hasValue(tableElement,rowNr,fieldid) {
              var bodyRows = $(tableElement).find('tbody tr[role="row"]');
              var field;
              if (bodyRows.length>rowNr) {
                field = $(bodyRows[rowNr]).find('.csui-table-cell-'+fieldid);
                if (field.length>0) {
            ...
            var expCols = $(tableView.$el).find('thead tr [class^="csui-table-cell-"]');
            var toggleCols = $(tableView.$el).find('thead tr [class^="csui-table-cell-_toggledetails"]');
            var detailsFields = $(tableView.$el).find('tbody tr.csui-details-row:first-child label');
            expect(expCols.length-toggleCols.length+detailsFields.length).toEqual(9);
          });
        });
      ...
      describe("support workspaces order", function () {
        ...
        function validateOrder(tableView, property, count, firstValue, lastValue) {
          ...
          expect($(element[0]).find(selectorExpandedViewCellName)[0].text).toEqual(firstValue);
          expect($(element[count -
                           1]).find(selectorExpandedViewCellName)[0].text).toEqual(lastValue);
        }
        ...
        function test_sorting(done,key) {
          var tableHead     = tableView.$(expandedExpandedTableHead)[0],
              tableHeadName = $(tableHead).find(selectorExpandedColumn + key);
          if (key !== 'name') {
            tableHeadName.click();
        ...
        it("server side via click on 'name'", function (done) {
      ...
      describe("filtering only allowed", function () {
        ...
        it("for strings", function () {
          ...
          expect($(tableView.$(selectorExpandedTableItemContainer)[0]).find(
              selectorExpandedViewCellName)[0].text).toEqual(title +
                                                             firstId);
          expect(tableView.$(selectorExpandedColumn + 'name').length).toBeGreaterThan(0);
          expect(tableView.$(
              selectorExpandedColumn + 'name .csui-table-column-search').length).toBeGreaterThan(0);
          expect(tableView.$(selectorExpandedColumn + '123_3').length).toBeGreaterThan(0);
          expect(
              tableView.$(selectorExpandedColumn + '123_3 .csui-table-column-search').length).toBe(
              0);
        });
      });
    });
