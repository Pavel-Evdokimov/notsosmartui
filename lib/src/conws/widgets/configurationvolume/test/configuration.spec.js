...
define([
  ...
  describe('Configuration volume view', function () {
    ...
    it("has 9 configured volumes", function () {
      expect(
          widget.$el.find(".csui-configurationvolume-container.csui-shortcut-region").length).toBe(
          9);
    });

    it("has correct configured volume name", function () {
      var volume1 = widget.$el.find(".csui-configurationvolume-container.csui-shortcut-region")[0];
      expect($(volume1).find(".csui-heading").text()).toBe("Water Management");
    });
