              ...
              },
              {
                ...
                setup: function (selectedTable) {
                  if (selectedTable.$.style.border) {
                    var border = selectedTable.$.style.border.split('px')[0];
                ...
                commit: function (data, selectedTable) {
                  ...
                  var currentValue = !!this.getValue() ? this.getValue() : 0;
                  if (currentValue === 0) {
                    if (selectedTable.$.style.removeProperty) {
                      selectedTable.$.style.removeProperty('border');
                    } else if (selectedTable.$.style.removeAttribute) {
                      selectedTable.$.style.removeAttribute('border');
                    }
                  } else {
                    selectedTable.$.style.border = currentValue + 'px solid';
                  }
                }
              },
              {
                ...
                setup: function (selectedTable) {
                  if (selectedTable.$.style['text-align']) {
                    var textAlign = selectedTable.$.style['text-align'];
                ...
                commit: function (data, selectedTable) {
                  ...
                  if (this.getValue()) {
                    selectedTable.$.style['text-align'] = this.getValue();
                  }
                }
              }]
          },
            {
              ...
              children: [{
                ...
                children: [{
                  ...
                  'default': editor.filter.check('table{width}') ?
                             (editable.getSize('width') < 500 ? '100%' : 500) : 0,
              ...
              },
                {
                  ...
                  children: [{
                    ...
                    getValue: defaultToPixel,
                    validate: CKEDITOR.dialog.validate.cssLength(
                        editor.lang.common.invalidCssLength.replace('%1',
                            editor.lang.common.height)),
                    onChange: function () {
                      var styles = this.getDialog().getContentElement('advanced', 'advStyles');
                ...
                },
                {
                  ...
                  setup: function (selectedTable) {
                    if (selectedTable.$.style['border-spacing']) {
                      var borderSpacing = selectedTable.$.style['border-spacing'].split('px')[0];
                      this.setValue(borderSpacing);
                    } else {
                  ...
                  commit: function (data, selectedTable) {
                    ...
                    if (currentValue === 0) {
                      if (selectedTable.$.style.removeProperty) {
                        selectedTable.$.style.removeProperty('border-spacing');
                      } else if (selectedTable.$.style.removeAttribute) {
                        selectedTable.$.style.removeAttribute('border-spacing');
                      }
                    } else {
                      selectedTable.$.style['border-spacing'] = currentValue + 'px';
                    }
                  }
                },
                {
                  ...
                  setup: function (selectedTable) {
                    if (selectedTable.$.rows[0].firstElementChild.style.padding) {
                      var borderPading = selectedTable.$.rows[0].firstElementChild.style.padding.split(
                          'px')[0];
                      this.setValue(borderPading);
                    } else {
                  ...
                  commit: function (data, selectedTable) {
                    ...
                    var currentValue = !!this.getValue() ? this.getValue() : 0,
                        table        = selectedTable.$;
                    // need some time to apply styling to table's cells as first time
                    // these cells are not available.
                    setTimeout(function () {
                      for (var i = 0, row; row = table.rows[i]; i++) {
                        for (var j = 0, cell; cell = row.cells[j]; j++) {
                          if (currentValue === 0) {
                            ...
                            } else if (cell.style.removeAttribute) {
                              cell.style.removeAttribute('padding');
                            }
                          } else {
                            cell.style.padding = currentValue + 'px';
                          }
                        }
                      }
                    }, 500);
            ...
            children: [{
              ...
              setup: function (selectedTable) {
                ...
                if (nodeList.count() > 0) {
                  ...
                  var firstElementChild = caption.getFirst(
