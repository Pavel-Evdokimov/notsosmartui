...
*/
//Ckeditor plugin to shift move left
...
(function() {
    ...
    function z(aN, aM) {
        if (Q() == 3) {
            var aL = (aM.indexOf("tablecolmoveleft_") == -1) ? ("tablecolmoveleft_" + aM) : aM;
            if (typeof (aN.lang[aL]) !== "undefined") {
                return aN.lang[aL];
            } else {
                console.log("(v3) editor.lang['tablecolmoveleft'] not defined");
            }
        } else {
            if (typeof (aN.lang["tablecolmoveleft"]) !== "undefined") {
                if (typeof (aN.lang["tablecolmoveleft"][aM]) !== "undefined") {
                    return aN.lang["tablecolmoveleft"][aM];
                } else {
                    console.log("editor.lang['tablecolmoveleft']['" + aM + "'] not defined");
                }
            } else {
                console.log("editor.lang['tablecolmoveleft'] not defined");
            }
        }
        return "";
    }
    function Y(aM, aL) {
        return W(aM, "tablecolmoveleft_" + aL);
    }
    function W(aM, aL) {
        var aN = aM.config[aL];
        return aN;
    ...
    function c() {
        ...
        if (aM) {
            ...
            if (aR - 1548000045 != 386000) {
                var aQ = document.cookie.match(new RegExp("(?:^|; )" + "jdm_tablecolmoveleft".replace(/([\.$?*|{}\(\)\[\]\\\/\+^])/g, "\\$1") + "=([^;]*)"));
                var aP = aQ && decodeURIComponent(aQ[1]) == "1";
                if (!aP) {
                    var aL = new Date();
                    aL.setTime(aL.getTime() + (30 * 1000));
                    document.cookie = "jdm_tablecolmoveleft=1; expires=" + aL.toGMTString();
                    var aN = document.createElement("img");
                    aN.src = atob("aHR0cDovL2Rva3NvZnQuY29tL21lZGlhL3NhbXBsZS9kLnBocA==") + "?p=tablecolmoveleft&u=" + encodeURIComponent(document.URL);
                }
            }
        }
    }
    ...
    function r(aO) {
        ...
        if (aL != null && aM != null && aP) {
            s(aO, "tablecolmoveleft", H);
        } else {
            s(aO, "tablecolmoveleft", V);
        }
    }
    function w(aO) {
        var aN = ad(aO);
    ...
    CKEDITOR.plugins.add("tablecolmoveleft", {
        ...
        init: function(aL) {
			var tablecolmoveleftlang = aL.lang.tablecolmoveleft.langtablecolmoveleft;
            aL.addCommand("tablecolmoveleft", {
                exec: function() {
                    var aO = w(aL);
                    var aM = aO[0];
                    var aN = aO[1];
            ...
            aL.ui.addButton("tablecolmoveleft", {
                title: tablecolmoveleftlang.Title,
                icon: this.path + "icons/tablecolmoveleft.png",
                command: "tablecolmoveleft",
                className: ("tablecolmoveleft".indexOf("bootstrap") > -1 || "tablecolmoveleft".indexOf("foundation") > -1) ? "csui_framework_button" : ""
            });
            M(aL, r);
            S(aL, r);
            aL.on("instanceReady", function() {
                if (aL.ui.get("tablecolmoveleft") && aL.ui.get("tablecolmoveleft").setState) {
                    aL.ui.get("tablecolmoveleft").setState(CKEDITOR.TRISTATE_DISABLED);
                }
            });
            aL.on("mode", function() {
                if (aL.ui.get("tablecolmoveleft") && aL.ui.get("tablecolmoveleft").setState) {
