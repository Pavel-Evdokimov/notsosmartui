...
define([
  ...
  describe("MetaDataView Widget", function () {
    ...
    describe("tab links", function () {
      ...
      xit('shows add category dialog', function (done) {
      ...
      describe('remove category', function () {
        ...
        it("Turn on the required field switch ", function () {
          if (v3.tabContentHeader && v3.tabContentHeader.requiredFieldSwitchView) {
        ...
        it('Switch should be turned to off state after clicking the confirm delete button',
            function (done) {
              asyncTestUtils.asyncElement($.fn.binf_modal.getDefaultContainer(),
                  '.csui-alert.binf-modal .binf-modal-footer .binf-btn-primary').done(
                  function (el) {
                    el.trigger('click');
                    expect(v3.$el.find('div .required-field-switch').attr('class')).not.toContain(
                        '.binf-switch-off');
                    done();
                  });
            });
    ...
    xit('For Folder thumbnail_section icon should be present', function () {
