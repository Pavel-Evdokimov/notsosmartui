  ...
  var NodesTableView = Marionette.LayoutView.extend({
    ...
    _updateZeroRecordsMessage: function () {
      this.tableView.setCustomLabels({
        zeroRecords: !!this.canDragAndDrop() && lang.dragAndDropMessage
      });
    },

    canDragAndDrop: function() {

      var retValue = (!this.container || this.container.get('type') !== 899) &&
                     this.dragNDrop && this.dragNDrop.canAdd();
      this.tableView.options.canDragAndDrop = retValue;
      return retValue;
    },
    ...
    _removeDragDropBorder: function (options) {
      ...
      options && options.highlightedTarget ? $(options.highlightedTarget).removeClass('drag-over') :
    ...
    addFilterCommandAria: function () {
      if (this.tableToolbarView && this.tableToolbarView.filterToolbarView) {
        ...
        if (this.ui.facetView.hasClass('csui-facetview-hidden')) {
