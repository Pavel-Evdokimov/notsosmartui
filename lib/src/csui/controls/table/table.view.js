...
define(['module',
  ...
  var TableView = Marionette.View.extend({
    ...
    constructor: function TableView(options) {
      ...
      $(window).bind("resize.tableview." + this.cid,
          _.debounce(_.bind(this._adjustColumnsIfViewResized, this), 50));
    ...
    destroy: function () {
      $(window).unbind("resize.tableview." + this.cid);
      ...
      if (this._renderingTableDelayedAsync) {
        ...
        if (this._renderTableDelayed) {
          ...
          if (this._canMeasureTableSize()) {
            ...
            log.warn("WARNING: onDomRefresh: el is still not measurable -> skip") &&
            console.log(log.last);
          }
        } else {
    ...
    _renderTable: function () {
      ...
      this._previousViewWidth = this.$el.outerWidth();  // for _viewChanged with threshold
      this.isRendered = true;
      ...
      for (var colno = 0; colno < this.allColumns.length; colno++) {
        ...
        if (column.attributes) {
          if (!column.attributes.permanentColumn) {
            ...
            if (column.CellView.flexibleWidth && tableEl) {
    ...
    _adjustColumnsIfViewResized: function () {
      if (this.isRendered) {  // table must be rendered, otherwise we don't care a. resizing
        var actualWidth = this.$el.outerWidth();
        actualWidth -= $.scrollbarWidth();
        if (actualWidth && actualWidth != this._previousViewWidth) {
          var upperLimit = this._previousViewWidth * 1.1;
          var lowerLimit = this._previousViewWidth * 0.9;
    ...
    showOrHideZeroRecordsMessage: function () {
      ...
      if (this.collection.length === 0) {
        ...
        var typeName       = !!this.collection.node && !!this.collection.node.get('type_name') ?
                             this.collection.node.get('type_name') : 'default',
            emptyTableText = "";
        typeName = typeName.replace(/\s+/g, '-').toLowerCase();
        this.$el.addClass("csui-table-empty");

        if (this._showEmptyViewText) {
          var labels = this.options.customLabels ? this.options.customLabels :
                       this.localizableLabels;
          emptyTableText = (!!this.collection.filters && this.collection.filters.name ?
                            labels.emptyTable : labels.zeroRecords);
        }

        var classNameToBeAdd = "csui-table-empty csui-table-empty-default csui-table-empty-" +
                               typeName +
                               (!!this.options.canDragAndDrop ? " csui-can-add-items" : ""),
      ...
      if (accessibleTable) {
        ...
        if (this.options.descriptionRowView) {
          if (aData.get('description') && aData.get('description').length > 0) {
            ...
            descriptionRowView.triggerMethod('row:toggle', !!this.detailsRowsAreExpanded);
          ...
          if (column.CellView) {
            ...
            if (column.name === '_toggledetails') {
              self.listenTo(cellView, 'toggle:detailsrow', function (args) {
                var descriptionRow = _.find(this._detailRowsDescriptionTRs,
                    function (description) {return description.view.model.cid === args.model.cid});
                descriptionRow &&
                descriptionRow.view.trigger("row:toggle", args.detailsRowIsExpanded);
