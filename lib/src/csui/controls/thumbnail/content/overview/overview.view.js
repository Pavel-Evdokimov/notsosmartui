  ...
  var OverviewView = Marionette.LayoutView.extend({
    ...
    onShowOverviewFlyout: function (args) {
      ...
      if (popoverMetadata.height() > window.innerHeight) {
        flyOutTopPosition = 0;
        popoverMetadataHeight = window.innerHeight -
                                (popoverName.height() + popoverActions.height());
        popoverMetadata.css({'height': popoverMetadataHeight});
      }
      ...
      if (isRtl && isWidthDiff) {
        ...
        if (this.placementChanged) {
          flyOutLeft = widthDiff + flyOutTarget.offset().left - popoverContainer.outerWidth();
        } else {
          flyOutLeft = widthDiff + flyOutTarget.offset().left + flyOutTarget.width();
        }
      } else {
        if (this.placementChanged) {
      ...
      if (documentHeight < flyOutTargetTop + flyOutHeight) {
        if (base.isIE11()) {
          flyOutTopPosition = $('nav.csui-navbar').outerHeight() + perspectivePanelClientTop;
          popoverArrowTop = (flyOutTarget.offset().top - flyOutTopPosition) + 12;
          popoverArrowEl.css('top', popoverArrowTop);
          popoverContainer.css({'top': flyOutTopPosition});
          popoverMetadata.height(popoverMetadata.height() - $('nav.csui-navbar').outerHeight());
          if (window.innerHeight < popoverMetadata.height() + popoverName.height() +
                                   popoverActions.height() + 100) {
            popoverMetadata.height(popoverMetadata.height() - 150);
          }
        } else {
          popoverMetadata.height(popoverMetadata.height() - $('nav.csui-navbar').outerHeight());
          if (window.innerHeight < popoverMetadata.height() + popoverName.height() +
                                   popoverActions.height() + 100) {
            popoverMetadata.height(popoverMetadata.height() - 100);
          }
        }
      }
