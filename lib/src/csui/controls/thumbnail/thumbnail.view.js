...
define([
  ...
  var ThumbnailItemView = Marionette.LayoutView.extend({
    ...
    onRender: function (e) {
      ...
      if (this.getSelectedChildren().length > 0 && self.model.isLocallyCreated && !self.model.inlineFormView) {
        this.$el.find('.csui-new-item').parent().find('.csui-thumbnail-select').addClass('csui-checkbox');
      }
      ...
      if (ThumbnailContent && ThumbnailContent.models) {
        _.each(ThumbnailContent.models, function (model, index) {
          if (this.options.model && this.options.model.isReservedClicked || this.options.model.isUnreservedClicked) {
            this.options.model.set('inactive', this.options.model.attributes.inactive, {silent: true});
          } else {
            this.options.model.set('inactive', !this.options.thumbnailView.checkModelHasAction(this.model), {silent: true});
          }
          var content = ContentFactory.getContentView(model);
          if (content) {
            var region        = model.get("key"),
  ...
  var ThumbnailListView = Marionette.CollectionView.extend({
    ...
    _onChildRender: function (childView) {
      childView.options.thumbnailView && childView.options.thumbnailView.trigger("thumbnailRowRendered", {
  ...
  var ThumbnailHeaderView = Marionette.LayoutView.extend({
    ...
    onSortClick: function () {
        if (this.options.thumbnailView.currentItemView && this.options.thumbnailView.currentItemView.overviewPopoverOpened) {
          this.options.thumbnailView.currentItemView.overview.currentView._closePopover();
        } else if (this.options.thumbnailView.currentItemView && this.options.thumbnailView.currentItemView.favoritePopoverOpened) {
    ...
    sortmenurender: function (name) {
      ...
      if (this._isRendered && this.options.thumbnailView.getSelectedChildren().length === 0 &&
          this.options.thumbnailView.resultsView.$el.find('.csui-checkbox')) {
        this.options.thumbnailView.resultsView.$el.find('.csui-checkbox').removeClass('csui-checkbox');
      }
    },
  ...
  var ThumbnailView = Marionette.LayoutView.extend({
    ...
    _updateSelectedChildren: function () {
      ...
      if (this.options.selectedChildren && this.options.selectedChildren.enableNonPromotedCommands) {
