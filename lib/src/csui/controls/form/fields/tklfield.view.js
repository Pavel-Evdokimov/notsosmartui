    ...
    accessibility: function (event) {
      ...
      if (validKeys.indexOf(event.keyCode) !== -1) {
        ...
        if (event.keyCode === 38 || event.keyCode === 40 || selected) {
          var ele = $(searchBase[index].children[0]).parent();
          if (!selected) {
            this._scrollIntoView(ele);
          }
          event.stopPropagation();
          event.preventDefault();
        }
      ...
      if (!this.alpacaField.schema.readonly) {
        ...
        if (!this.valuesPulled) {
          ...
          var fullUrl,
              ...
              index          = 0,
              isValidRequest = true,
              userFieldTypes = ['otcs_user_picker', 'otcs_member_picker', 'otcs_user'],
              parentPropertyId,
              parentViews,
              multiFieldParents,
              that           = this;
            ...
            if (this.alpacaField.options.isMultiFieldItem) {
              ...
              if (this.alpacaField.options) {
                ...
                this.parentViews.map(function (parent) {
                  ...
                  if (!!parentViews[index]) {
                    ...
                    if (!parentViews[index].isNonTKLField) {
                      attribute_values[parentPropertyId] = parentViews[index].getValue();
                    } else {
                      if (userFieldTypes.indexOf(parentViews[index].alpacaField.schema.type) !== -1) {
                        attribute_values[parentPropertyId] = parentViews[index].getEditValue().id;
                      } else {
                        attribute_values[parentPropertyId] = parentViews[index].getDisplayValue();
                      }
                    }
                  } else {
                    if (!this.isMultiFieldView(parent)) {
                      ...
                      if (!parentViews[0].isNonTKLField) {
                        attribute_values[parentPropertyId] = parentViews[0].getValue();
                      } else {                                                
                        if (userFieldTypes.indexOf(parentViews[index].alpacaField.schema.type) !== -1) {
                          attribute_values[parentPropertyId] = parentViews[0].getEditValue().id;
                        } else {
                          attribute_values[parentPropertyId] = parentViews[0].getDisplayValue();
                        }
                      }
                    } else {
              ...
              for (index = 0; index < this.parentViews.length; index++) {
                ...
                if (!parentViews[0].isNonTKLField) {
                  ...
                  if (userFieldTypes.indexOf(parentViews[0].alpacaField.schema.type) !== -1) {
                    attribute_values[parentPropertyId] = parentViews[0].getEditValue().id;
                  } else {
                    attribute_values[parentPropertyId] = parentViews[0].getDisplayValue();
                  }
