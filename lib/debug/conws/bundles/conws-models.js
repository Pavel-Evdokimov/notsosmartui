  ...
  var ParticipantModel = NodeModel.extend({
    ...
    save: function (data, options) {
      ...
      _.each(data.add, function (role) {
        ...
        // do the AJAX call
        $.ajax(options).done(_.bind(function (response) {
              successAdd.push({
        ...
        parse: function (response) {
          ...
          var metadata = {
            conws_roleavatar: Metadata.integer(
                RolesTableColumnCollection.columnNames.avatar, ''),
...
csui.define('conws/models/categoryforms/categoryforms.model',["module", "csui/lib/jquery", "csui/lib/underscore", "csui/lib/backbone",
    ...
    var CategoryFormCollection = Backbone.Collection.extend({
        ...
        _getMetadataForms: function (method, model, options) {
            ...
            $.ajax(this.connector.extendAjaxOptions({
                type: 'GET',
                url: catsUrl
            }))
...
csui.define('conws/utils/workspaces/impl/workspaces.collection.mixin',['module', 'csui/lib/jquery', 'csui/lib/underscore', 'csui/lib/backbone', 'csui/utils/url',
  ...
  var WorkspacesColumnCollection = NodeChildrenColumnCollection.extend({
    ...
    constructor: function WorkspacesColumnCollection(dataCollectionName) {
    ...
    mixin: function(prototype) {
      ...
      return _.extend(prototype,{
        ...
        makeWorkspacesCollection: function (options) {
          this.columns = new WorkspacesColumnCollection(this.dataCollectionName);
        ...
        parse: function (response) {
          ...
          this.titleIcon = response.wksp_info.wksp_type_icon;
            ...
            for (var name in this.filters) {
              if (this.filters.hasOwnProperty(name)) {
                if (this.filters[name] === "") {
                  ...
                  queryParams["where_" + name] = "contains_" + encodeURIComponent(this.filters[name]);
                }
              }
            }
          }
            ...
            for (var i = 0; i < filterConfig.rules.length; i++) {
                ...
                if (ruleMatch) {
                    try {
                        ...
                        if (retrieves && retrieves.length > 0) {
                            var term = retrieves.length > 1 ? retrieves[1] : retrieves[0];
                            filters.push({ typeId: rule.wkspTypeId, filterName: term, weight: rule.weight });
                        }
                    } catch (e) {
...
csui.define('conws/widgets/outlook/impl/nls/root/lang',{
    ...
    error_save_email: "The saving process did not complete successfully with message: {0}",
    ...
    save_selection_conflict_msg: "File(s) with the same name already exists. Please rename the file(s) to continue.",
