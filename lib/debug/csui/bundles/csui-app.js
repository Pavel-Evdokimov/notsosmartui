...
csui.define('csui/controls/tab.panel/tab.links.ext.scroll.mixin',['csui/lib/underscore', 'csui/lib/jquery', 'csui/lib/backbone', 'csui/models/version',
  ...
  var TabLinksScrollMixin = {
    ...
    _listenToTabIdEvent: function (index, iTabs) {
      var self = this;
      var tabs = iTabs === undefined ? this.tablinksToolbar.find(this.tabsSelector) : iTabs;
    ...
    _gotoPreviousTab: function (options) {
      ...
      var tabs = options.tabs === undefined ? this.tablinksToolbar.find(this.tabsSelector) : options.tabs;
  ...
  var BreadcrumbsPanelView = Marionette.LayoutView.extend({
    ...
    _getAncestors: function () {
      ...
      if (this.ancestors !== ancestors) {
        // perspective changed -> new ancestors
        if (this.ancestors) {
    ...
    _ancestorsFetched: function () {
      ...
      // let breadcrumb update immediately only if context fetch finished - otherwise delay it
      // until context fetch has finished
  ...
  var BreadcrumbsView = Marionette.ItemView.extend({
    ...
    constructor: function BreadcrumbsView(options) {
      ...
      this.listenTo(this.applicationScope, 'change:breadcrumbsAvailable',
          this._breadcrumbAvailibilityChanged);
      this._breadcrumbVisibilityChanged();
      this._breadcrumbAvailibilityChanged();
    },
    ...
    _breadcrumbAvailibilityChanged: function () {
  ...
  var ZonePerspectiveView = GridView.extend({
    ...
    serializeOptions: function (perspectiveModel) {
      ...
      $.whenAll.apply($, widgetPromises).done(function (results) {
        ...
        self.executeCallbacks(widgets, perspectiveModel).done(function () {
...
csui.define('csui/perspectives/left-center-right/left-center-right.perspective.view',['require', 'module', 'csui/lib/underscore', 'csui/perspectives/zone/zone.perspective.view',
  ...
  var LeftCenterRightPerspectiveView = ZonePerspectiveView.extend({
    ...
    serializePerspective: function (perspectiveModel) {
      ...
      return optionsPromise.then(function (results) {
        var constant_data = _.flatten(results.map(function (result) {
              return result.config.constantsData || [];
            })),
            options       = _.reduce(results, function (options, result) {
              options[result.zone] = result.config.widget;
              return options;
            }, {});
        return {
          perspective: {
    ...
    render: function () {
      ...
      this.listenTo(this.tabLinks, 'before:edit', function (tab) {
        ...
        this[scroll]().done(_.bind(function () {
          ...
          tab && tab.ui && tab.ui.editInput.focus();
        }, this));
      });
      return this;
    },
  ...
  var ShortcutView = Marionette.ItemView.extend({
    ...
    getName: function() {
      if(( this.options.data.displayName || "" ).trim().length > 0) {
        ...
        return this.model.fetched ? this.model.get('name') : lang.loadingText;
      }
    }
...
csui.define('csui/widgets/shortcuts/shortcuts.view',[
  ...
  var ShortcutsView = Marionette.LayoutView.extend({
    ...
    _setupItems: function() {
      ...
      self._items = _.map(_.first(this.options.data.shortcutItems, 4), function(item, index, arr) {
        ...
        return {
          ...
          type: item.type,
          layout: layout,
          shortcutView: self._getShortcutView(options, self.options.context)
        };
      });
    },
    ...
    _getShortcutView: function(options, context) {
