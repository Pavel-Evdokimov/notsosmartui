...
CKEDITOR.dialog.add( 'anchor', function( editor ) {
	...
	return {
		...
		onShow: function() {
            var sel = editor.getSelection(),
               // [OT] Modifications BEGIN
               //The changes are done to fix one scenario i.e able to attach a link and anchor to an image
                element=sel.getSelectedElement(),
				fullySelected = ( element && element.type === CKEDITOR.NODE_ELEMENT &&
                    ( element.data( 'cke-real-element-type' ) === 'anchor' || element.is( 'a' ) ) )?element:undefined,
               // [OT] Modifications END 
