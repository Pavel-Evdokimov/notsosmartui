...
CKEDITOR.dialog.add( 'wikilink', function( editor )
{
  ...
  {
    var dialog = this.getDialog(),
        partIds = [ 'wikilinkOptions', 'urlOptions', 'anchorOptions', 'emailOptions' ],
        typeValue = this.getValue(),
        uploadTab = dialog.definition.getContents( 'upload' ),
        uploadInitiallyHidden = uploadTab && uploadTab.hidden;

    // hide the Advanced tab for wiki link
    if ( typeValue == 'wikilink' )
  ...
  var parseLink = function( editor, element )
  {
    ...
    if ( !retval.type )
    {
      ...
      {
        // WIKI-588 Required input argument "func" not specified when clicking on certain wiki links (Displaying wikilinks)
        var res = urlMatch[2].split("/");
        if(res[res.length-2] == "wiki")
          urlMatch[2] = editor.config['llBase'] + '/wiki/' + editor.config.wikilink.wikiId + '/' + res[3];
  ...
  var commonLang = editor.lang.common,
      ...
      wikiLinkLang = editor.lang.wikilink.langWikiLink;  //object structured changed with the latest update(CKEditor 4.4.3) of CKEditor


  var llBase = editor.config['llBase'];
  var nodeId = editor.config.wikilink.wikiId;


  function OpenBrowserWindow( url, width, height )
  {
    var iLeft = ( editor.config.width - width ) / 2;
    var iTop  = ( editor.config.height - height ) / 2;
  ...
  return {
            ...
            [
              {
                ...
                items :
                    [
                      [ wikiLinkLang.wikilink, 'wikilink' ],
                      [ commonLang.url, 'url' ],
                            ...
                            [
                              {
                                ...
                                items :
                                    [
                                      [ wikiLinkLang.wikiPageHere ],
                                      [ wikiLinkLang.wikiCSObject ]
                                    ],
                                commit : function( data )
                                {
                                  data.wikilinkRadio = this.getValue();
                                }
                              }
                            ]
                      },
                                    ...
                                    [
                                      {
                                        ...
                                        onClick : function()
                                        {
                                          var wikilinkRadio = this.getDialog().getContentElement( 'info', 'wikilinkRadio' );
                                          if ( wikilinkRadio.getValue() != wikiLinkLang.wikiPageHere )
                                          {
                                            wikilinkRadio.setValue( wikiLinkLang.wikiPageHere );
                                          }
                                        },
                                        validate : function()
                                        {
                                          ...
                                          if ( dialog.getContentElement( 'info', 'linkType' ) &&
                                               dialog.getValueOf( 'info', 'linkType' ) != 'wikilink' )
                                            return true;

                                          if ( this.getDialog().fakeObj )	// Edit Anchor.
                                            return true;

                                          var wikilinkRadio = dialog.getContentElement( 'info', 'wikilinkRadio' );

                                          if ( wikilinkRadio.getValue() == null )
                                          {
                                            var func = CKEDITOR.dialog.validate.notEmpty( wikiLinkLang.wikilinkError );
                                        ...
                                        onLoad: function()
                                        {
                                          editor.config.wikilink.addItems();
                                        },
                                        setup : function( data )
                                        {
                                          var select = this.getDialog()._.contents.info.pageHereText.getInputElement();
                                          select.$.options.selectedIndex = -1;

                                        },
                                        commit : function( data )
                                        {
                                          data.wikipageName = this.getValue();
                                        }
                                      }
                                    ]
                              },
                                    ...
                                    [
                                      {
                                        ...
                                        {
                                          var wikilinkRadio = this.getDialog().getContentElement( 'info', 'wikilinkRadio' );
                                          if ( wikilinkRadio.getValue() != wikiLinkLang.wikiCSObject )
                                          {
                                            wikilinkRadio.setValue( wikiLinkLang.wikiCSObject );
                                          }
                                          this.getDialog().getContentElement( 'info', 'csObjectBrowse' ).focus();
                                        },
                                        validate : function()
                                        {
                                          var dialog = this.getDialog();
                                        ...
                                        setup : function( data )
                                        {
                                          if ( data.csObjectName )
                                            this.setValue( data.csObjectName );
                                        },
                                        commit : function( data )
                                        {
                                          data.csObjectName = this.getValue();
                                        }
                                      },
                                      {
                                        type : 'button',
                                        ...
                                        onClick : function()
                                        {
                                          ...
                                          editor.config.wikilink.openTargetPicker();
                                        }
                                      },
                                      {
                                        type : 'text',
                                        ...
                                        {
                                          if ( data.csObjectId )
                                            this.setValue( data.csObjectId );
                                        },
                                        commit : function( data )
                                        {
                                          data.csObjectId = this.getValue();
                                        }
                                      }
                                    ]
                              }
    ...
    onOk : function()
    {
      ...
      case 'wikilink':
        var linkId, link;
        if ( data.wikilinkRadio == wikiLinkLang.wikiPageHere )
        {
          browsedObjName = data.wikipageName;
          var temp = browsedObjName;
          ...
          //Required input argument "func" not specified when clicking on certain wiki links. WIKI-588
          link = editor.config['llBase'] + '/wiki/' + editor.config.wikilink.wikiId + '/' + encodeURIComponent(browsedObjName);
        }
        else
        {
          browsedObjName = data.csObjectName;
          linkId = data.csObjectId;
          link = editor.config['llBase'] + '/open/' + linkId;
        }
        attributes["data-cke-saved-href"] =  ( link );
        break;
      case 'url':
      ...
      if ( !this._.selectedElement )
      {
        ...
        var linkattrs = plugin.getLinkAttributes(editor,attributes)
        if ( ranges.length == 1 && ranges[0].collapsed )
        {
          var text;
          if ( browsedObjName != "" )
          {
            text = new CKEDITOR.dom.text( browsedObjName, editor.document );
          }
          else
          {
        ...
        {
          selection = selection.getRanges()[0], selection.collapsed && (a = new CKEDITOR.dom.text("email" == attributes.type ? attributes.email.address : linkattrs.set["data-cke-saved-href"], editor.document), selection.insertNode(attributes), selection.selectNodeContents(attributes))
        }
        if(selection instanceof CKEDITOR.dom.selection)
        {
          selection = selection.getRanges()[0];
        }
        // Apply style.
        var style = new CKEDITOR.style( { element : 'a', attributes : attributes } );
        style.type = CKEDITOR.STYLE_INLINE,		// need to override
            style.applyToRange( selection,editor ),
            selection.select();
      }
      else
      {
    ...
    onLoad : function()
    {
      // Fire the link type dropdown's onchange handler on startup to show only the relevant controls
      this.getContentElement("info", "linkType").fire("change");
